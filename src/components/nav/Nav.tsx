...
const Nav: React.FunctionComponent<NavBarProps> = ({ brandName }) => {
    ...
    return (
        <section className="navbar">
            <nav className="w-full relative py-3 mb-3 shadow-lg">
                <div className="flex  items-center justify-between">
                    <div className="w-full flex justify-between md:w-auto px-4">
                        <div className="flex">
                            ...
                            <a className="text-sm font-bold leading-relaxed inline-block mr-4 py-2 whitespace-no-wrap uppercase text-gray-500" href="/">
                                {brandName}
                            </a>
                        </div>
                        ...
                        <ul className="flex flex-col md:flex-row list-none ml-auto">
                            {
                                menuItems && menuItems.map(x => <li className="px-4" key={x.key}>
                                    <a target="_blank" rel="noopener noreferrer" href={x.url} className="px-3 py-2 flex text-xs uppercase font-bold leading-snug text-gray-800 hover:opacity-75">{x.name}</a>
                                </li>)
                            }
                        </ul>
                    </div>
